        ...
        cars_detali = {
            ...
            view_cars_not_outgoing: function (list) {
                ...
                $('#list-cars-not-outgoing a').on('click', function (e) {
                    ...
                    cars_detali.select_id_outgoing_detention_return = null; // Сбросим id задержания
                    ...
                    cars_detali.ids_inc.getOutgoingCarsOfID(id, function (car) {
                        if (car !== null) {
                            ...
                            cars_detali.select_id_outgoing_detention_return = car.id_outgoing_detention_return;     // Сохраним id задержания
                            ...
                            cars_detali.ids_inc.getWagonInternalRoutesOfOutgoingCarsID(id, function (wir) {
                                if (wir) {
                                    ...
                                    cars_detali.loadOutgoingDetentionReturnOfnum(cars_detali.select_num, function () {
            ...
            detention_save: $('button#detention_save').on('click', function () {
                ...
                if (valid) {
                    if (valid) {
                        ...
                        cars_detali.ids_inc.postOutgoingDetentionReturn(new_detention, function (result_add) {
                            if (result_add > 0) {
                                // Добавил задержанание
                                cars_detali.select_id_outgoing_detention_return = result_add; // Запомним новую ссылку на задержание
                                cars_detali.ids_inc.getOutgoingCarsOfID(cars_detali.select_id, function (car) {
                                    if (car !== null) {
                                        car.id_outgoing_detention_return = result_add;
                                        car.Directory_Wagons = null;
                                        car.WagonInternalRoutes = null;
                                        car.change = toISOStringTZ(new Date());
                                        car.change_user = cars_detali.user;
                                        cars_detali.ids_inc.putOutgoingCars(car, function (result_car_upd) {
                                            if (result_car_upd > 0) {
                                                // Обновим список задержаний
                                                cars_detali.loadOutgoingDetentionReturnOfnum(cars_detali.select_num, function () {
                                                ...
                                                cars_detali.val_outgoing_car_detention.out_error_message("Ошибка обновления строки вагона для предъявления, ссылка на строку таблицы задержания [IDS].[OutgoingDetentionReturn] с id=" + result_add + "- не добавлена!");
                                                LockScreenOff();
                                            }
                                        });
                                    } else {
                                        cars_detali.val_outgoing_car_detention.out_error_message("Ошибка обновления строки вагона для предъявления, строка таблицы [IDS].[OutgoingCars] c id=" + cars_detali.select_id + " не найдена");
                                        LockScreenOff();
                                    }
                                });

                            } else {
                                cars_detali.val_outgoing_car_detention.out_error_message("Ошибка добавления новой строки задержания!");
                                LockScreenOff();
                            }
                        });
            ...
            view_cars_detention_current: function () {
                if (cars_detali.list_detention_return && cars_detali.list_detention_return.length > 0) {
                    var detention = cars_detali.list_detention_return.find(function (o) {
                        return o.id === cars_detali.select_id_outgoing_detention_return && o.type_detention_return === 0;
                    });
                    ...
                    if (detention) {
                        ...
                        cars_detali.cause_detention.prop("disabled", false);
                        cars_detali.detention_start.obj.prop("disabled", false);
                        cars_detali.detention_stop.obj.prop("disabled", false);
                    }
                } else {
                    cars_detali.detention_save.show();
                    cars_detali.cause_detention.prop("disabled", false);
            ...
            // Загрузка задержаний по номеру вагона
