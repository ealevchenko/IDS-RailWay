        ...
        pn_select = {
            ...
            bt_delete_park_status: $('button#delete_park_status').on('click',
                function (event) {
                    ...
                    // Определим станцию
                    var station = pn_select.get_station_of_id(pn_select.id_station_select);
                    pn_select.get_park_state_of_id(pn_select.id_park_status_select, function (park_state) {
            ...
            get_park_state_of_id: function (id, callback) {
                if (id > 0) {
                    ids_inc.getParkState_StationOfID(id, function (park_state) {
                        if (typeof callback === 'function') {
                            callback(park_state);
                        }
                    })
            ...
            // Проверить применяется на данный момент положение парка по указаной станции
            is_apply_park_state_of_station: function (id_station, callback) {
                ids_gl.getParkStateApply(function (result_apply) {
                    if (result_apply && result_apply.length > 0) {
                        var apply = false;
                        $.each(result_apply, function (i, el) {
                            if (Number(el) === id_station) {
                                apply = true;
                            }
                        });
                        if (typeof callback === 'function') {
            ...
            // Сбросить признак идет применение положения парка по указаной станции
            clear_apply_park_state_of_station: function (id_station, callback) {
                ids_gl.deleteParkStateApply(id_station, function (result_delete) {
                    if (result_delete !== null) {
                        ...
                        if (typeof callback === 'function') {
                            pn_select.val_add_park_status.out_warning_message('Ошибка сброса на сервере бита «Идет применения положения парка»');
            ...
            validation_park_state: function (callback) {
                ...
                // Проверим на применение положения по станции
                pn_select.is_apply_park_state_of_station(Number(pn_select.id_station_select), function (run_apply) {
                    // Проверим ответ применяется?
                    if (run_apply === false) {
                        ...
                        //------------
                        if (id_park_status > 0) {
                            // Статус выбран, найдем строку статуса
                                    ...
                                    if (list_wagon && list_wagon.length > 0) {
                                        ...
                                        $.each(list_wagon, function (i, el) {
                                            if (el.id === id_park_status) {
                                                // принадлежит текущему положению
                                                list_internal.push(el);
                                            } else {
                                                // не принадлежит текущему положению
                                                list_external.push(el);
                                            }
                                        });
                                        // Разделили
                                        var list_wagon_busy = [];   // список вагонов запрет операций
                                        var list_move_busy = [];    // список вагонов запрет перемещений
                                        ...
                                        if (list_internal && list_internal.length > 0) {
                                            $.each(list_internal, function (i, el) {
                                                if (el.wir_id === null) {
                                                    ...
                                                    if (el.wim_close !== null) {
                                                        ...
                                                        //if (el.current_wagon_busy === 1) {
                                                        //    list_wagon_busy.push(el);
                                                        //}
                                                        //if (el.current_move_busy === 1) {
                                                        //    list_move_busy.push(el);
                                                        //}
                                                        //if (el.current_wagon_busy !== 1 && el.current_move_busy !== 1) {
                                                        wagons_amkr.push(el);
                                            ...
                                            // проверим вагоны 
                                            //if (list_wagon_busy && list_wagon_busy.length > 0) {
                                            //    valid = false;
                                            //    pn_select.val_add_park_status.out_error_message('Ошибка, найдено :' + list_wagon_busy.length + ' вагонов по которым запрет новых операций');
                                            //}
                                            //if (list_move_busy && list_move_busy.length > 0) {
                                            //    valid = false;
                                            //    pn_select.val_add_park_status.out_error_message('Ошибка, найдено :' + list_move_busy.length + ' вагонов по которым запрет перемещений');
                                            //}
                        ...
                        // Парк не выбран
                        pn_select.val_add_park_status.out_warning_message('Состояние парка по указанной станции уже применяется!');
                        if (typeof callback === 'function') {
                            callback(null);
                        }
                    }
                                    ...
                                    if (result_position && result_position.length > 0) {
                                        ...
                                        if (result_position[0].close_wir === null) {
                                            ...
                                            if (result_position[0].id_outer_way === null) {
                                                // Вагон на станции
                                                if (result_position[0].current_move_busy === 1) {
                                                    ...
                                                    if (result_position[0].id_operation_wagon === 9) {
                                                        ...
                                                        result_dislocation = '!ВНМАНИЕ ПО ВАГОНУ ЗАПРЕТ ПЕРЕМЕЩЕНИЯ, последняя операция: ' + result_position[0]['operation_wagon_name_' + lang] + ' находится на станции : ' + result_position[0]['station_name_' + lang] + '; <br/>Путь станции : ' + result_position[0]['way_num_' + lang] + ' - ' + result_position[0]['way_name_' + lang] + '; <br/>Позиция на пути : ' + result_position[0].position + ', прибыл на путь : ' + getReplaceTOfDT(result_position[0].way_start);
                                                    }
                                                } else {
        ...
        setInterval(function () {
            ...
            ids_gl.getParkStateApply(function (result_apply) {
                if (result_apply && result_apply.length > 0) {
                    ...
                    $.each(result_apply, function (i, el) {
                        if (Number(el) === id_station) {
                            $('label#curent_apply').text('ИДЕТ ПРИМЕНЕНИЕ ПОЛОЖЕНИЯ ПАРКА');
                        }
                    });
