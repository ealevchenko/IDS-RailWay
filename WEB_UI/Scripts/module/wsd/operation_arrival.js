...
(function (window) {
    ...
    $.Text_View =
    {
        ...
        {
            'card_header_panel': 'ИСТОРИЯ ОПЕРАЦИЙ "ПРИБЫТИЯ СОСТАВОВ НА СТАНЦИИ АМКР"',
    ...
    //
    function operation_arrival(selector) {
        if (!selector) {
    ...
    operation_arrival.prototype.init_columns = function () {
        var collums = [];
    ...
    operation_arrival.prototype.load_db = function (list, update, callback) {
        if (list) {
    ...
    operation_arrival.prototype.init = function (options, fn_init_ok) {
    ...
    // Показать данные 
    operation_arrival.prototype.view = function (operation) {
    ...
    // загрузить данные 
    operation_arrival.prototype.load_default = function () {
    ...
    // загрузить данные 
    operation_arrival.prototype.load_operation = function (start, stop) {
        if (start >= 0 && stop >= 0) {
            LockScreen(langView('mess_load_operation', App.Langs));
    ...
    // Обновить данные
    operation_arrival.prototype.update = function () {
        this.out_clear();
    ...
    // Показать
    operation_arrival.prototype.show = function () {
        this.$panel.show();
    }
    // Скрыть
    operation_arrival.prototype.hide = function () {
        this.$panel.hide();
    }
    // Очистить сообщения
    operation_arrival.prototype.out_clear = function () {
        if (this.settings.alert) {
            this.settings.alert.clear_message()
        }
    }
    // Показать ошибки
    operation_arrival.prototype.out_error = function (message) {
        if (this.settings.alert) {
            this.settings.alert.out_error_message(message)
        }
    }
    // Показать предупреждения
    operation_arrival.prototype.out_warning = function (message) {
        if (this.settings.alert) {
            this.settings.alert.out_warning_message(message)
        }
    }
    // Показать сообщения о выполнении действий
    operation_arrival.prototype.out_info = function (message) {
        if (this.settings.alert) {
    ...
    operation_arrival.prototype.detali_select_row = function (tr) {
    ...
    operation_arrival.prototype.init_detali = function () {
    ...
    // Показать детали
    operation_arrival.prototype.view_detali = function (data) {
        var base = this;
    ...
    operation_arrival.prototype.destroy_detali = function (data) {
    ...
    // Очистить все детали
    operation_arrival.prototype.destroy_all_detali = function () {
    ...
    // Очистить объект
    operation_arrival.prototype.destroy = function () {
    ...
    operation_arrival.prototype.destroy_table = function () {
        // Очистить объект таблица
        if (this.obj_t_sostav) {
            //LockScreen(langView('mess_destroy_operation', App.Langs));
            this.obj_t_sostav.destroy(true);
    ...
    App.operation_arrival = operation_arrival;
