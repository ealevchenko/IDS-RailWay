...
/*
Скрипт [get_view_open_arrival_sostav_of_outer_ways]
Выборка всех доконца не принятых составов на указаную станцию 
(отображаются все составы с учетом принятых и возврата)
...
CREATE FUNCTION [IDS].[get_view_open_arrival_sostav_of_outer_ways]
 (
		@id_station_on int
 )
		RETURNS 
...
BEGIN
	...
	from [IDS].[get_view_open_wagons_of_outer_ways_station_on](@id_station_on) as fwag
	group by fwag.outer_way_num_sostav
	HAVING count(fwag.outer_way_num_sostav) > (select IDS.[get_count_wagon_accepted_of_sostav_outer_ways](fwag.outer_way_num_sostav))
