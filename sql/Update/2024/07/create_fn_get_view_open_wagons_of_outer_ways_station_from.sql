...
/****** Object:  UserDefinedFunction [IDS].[get_view_open_wagons_of_outer_ways_station_from]    Script Date: 04.07.2024 15:23:02 ******/
SET ANSI_NULLS ON
GO
...
CREATE FUNCTION [IDS].[get_view_open_wagons_of_outer_ways_station_from]
 (
	@id_station_from int
 )
		RETURNS 
...
BEGIN
	...
	where wim_from.[num_sostav] in (select distinct [num_sostav] FROM IDS.WagonInternalMovement as wim Left JOIN IDS.Directory_OuterWays as ow ON wim.id_outer_way = ow.id WHERE wim.[num_sostav] is not null and ow.[id_station_from] = @id_station_from and wim.[outer_way_end] is null)
