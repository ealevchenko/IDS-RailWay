...
/****** Object:  UserDefinedFunction [IDS].[get_view_directory_wagon]    Script Date: 13.11.2020 17:11:20 ******/
SET ANSI_NULLS ON
GO
...
CREATE FUNCTION [IDS].[get_view_directory_wagon]
 (
  ...
  FROM .[IDS].[Directory_Wagons] as w Left JOIN
	IDS.[Directory_WagonsRent] as wr ON wr.id = (SELECT TOP (1) [id] FROM [KRR-PA-CNT-Railway].[IDS].[Directory_WagonsRent] where [num]= w.num order by id desc)  Left JOIN
