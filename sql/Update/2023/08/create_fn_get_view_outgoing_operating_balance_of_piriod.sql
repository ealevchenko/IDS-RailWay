...
/****** Object:  UserDefinedFunction [IDS].[get_view_outgoing_operating_balance_of_piriod]    Script Date: 07.08.2023 14:41:10 ******/
SET ANSI_NULLS ON
GO
...
CREATE FUNCTION [IDS].[get_view_outgoing_operating_balance_of_piriod]
 (
   @start datetime,
   @stop datetime
 )
	...
	SELECT out_car.num
		...
		--into outgoing_operating_balance
	FROM [IDS].[OutgoingSostav] as out_sost
		...
		Left JOIN [IDS].[WagonInternalRoutes] as wir ON wir.id_outgoing_car = out_car.id
