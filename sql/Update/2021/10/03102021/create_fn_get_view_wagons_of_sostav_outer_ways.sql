...
GO
--> Функция возвращает вагоны по указаному составу на перегоне
CREATE FUNCTION [IDS].[get_view_wagons_of_sostav_outer_ways]
 (
    @outer_way_num_sostav nvarchar(50)
 )
		RETURNS 
	@view_wagons TABLE  (
...
BEGIN
	...
	WHERE wim_from.[num_sostav] = @outer_way_num_sostav 
	RETURN
END
