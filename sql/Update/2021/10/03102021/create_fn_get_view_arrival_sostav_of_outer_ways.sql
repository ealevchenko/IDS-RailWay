...
/****** Object:  UserDefinedFunction [IDS].[get_view_arrival_sostav_of_outer_ways]    Script Date: 03.10.2021 22:00:16 ******/
SET ANSI_NULLS ON
GO
...
CREATE FUNCTION [IDS].[get_view_arrival_sostav_of_outer_ways]
 (
		@id_station_on int
 )
		RETURNS 
...
BEGIN
	...
	HAVING min(fwag.on_id_station) = @id_station_on 
	and count(fwag.outer_way_num_sostav) > (SELECT count([id]) FROM [IDS].[WagonInternalMovement] where [num_sostav] = fwag.outer_way_num_sostav and [outer_way_end] is not null)
	--and count(count_wagons_send) > count_wagons_arrival
