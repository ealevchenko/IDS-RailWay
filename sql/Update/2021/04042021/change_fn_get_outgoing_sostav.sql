...
/****** Object:  UserDefinedFunction [IDS].[get_outgoing_sostav]    Script Date: 04.04.2021 22:30:38 ******/
SET ANSI_NULLS ON
GO
...
drop FUNCTION [IDS].[get_outgoing_sostav]
go

CREATE FUNCTION [IDS].[get_outgoing_sostav]
 (
 )
	RETURNS 
...
SELECT
		  ...
		  ,count_outgoing = (SELECT count (AC.[id]) FROM [IDS].[OutgoingCars] AS AC where AC.[id_outgoing]=outs.id and AC.[position_outgoing] is not null and AC.parent_wir_id is null)
		  ,count_not_outgoing = (SELECT count (AC.[id]) FROM [IDS].[OutgoingCars] AS AC where AC.[id_outgoing]=outs.id and AC.[position_outgoing] is null and AC.parent_wir_id is null)
		  ,count_return = (SELECT count (AC.[id]) FROM [IDS].[OutgoingCars] AS AC where AC.[id_outgoing]=outs.id and AC.parent_wir_id is not null)
		  ,count_detention = (SELECT count (AC.[id]) FROM [IDS].[OutgoingCars] AS AC where AC.[id_outgoing]=outs.id and AC.id_outgoing_detention is not null and AC.parent_wir_id is null)
		  ,count_vagonnik = (SELECT count (AC.[id]) FROM [IDS].[OutgoingCars] AS AC where AC.[id_outgoing]=outs.id and AC.vagonnik is not null)
