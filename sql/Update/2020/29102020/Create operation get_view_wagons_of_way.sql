...
/****** Object:  UserDefinedFunction [IDS].[get_view_wagons_of_way]    Script Date: 02.11.2020 21:30:29 ******/
SET ANSI_NULLS ON
GO
...
CREATE FUNCTION [IDS].[get_view_wagons_of_way]
 (
	@id_way int
 )
		RETURNS 
		...
		FROM IDS.WagonInternalMovement as wim INNER JOIN
			...
			-- Справочник вагонов
			IDS.Directory_Wagons as dir_wagon ON wir.num = dir_wagon.num  INNER JOIN
			...
			-- Справочники 
			IDS.Directory_Countrys as dir_countrys ON dir_wagon.id_countrys = dir_countrys.id INNER JOIN								--> Страна вагона
			...
			IDS.Directory_LimitingLoading as dir_limload ON cur_dir_rent.id_limiting =  dir_limload.id INNER JOIN                       --> Текущее ограничение погрузки
			IDS.Directory_ConditionArrival as arr_dir_cond ON ar_doc_vag.id_condition =  arr_dir_cond.id INNER JOIN                     --> Техническое сотояние по прибытию
			IDS.Directory_Cargo as arr_dir_cargo ON ar_doc_vag.id_cargo =  arr_dir_cargo.id  INNER JOIN                                 --> Груз по прибытию
			...
			IDS.Directory_CertificationData as arr_dir_certif ON ar_doc_vag.id_certification_data =  arr_dir_certif.id INNER JOIN       --> Сертификат данные по прибытию
