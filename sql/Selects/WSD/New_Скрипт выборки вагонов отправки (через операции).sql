USE [KRR-PA-Test-Railway]
	...
	FROM IDS.WagonInternalOperation as wio_from --> операция отправка на станцию

	--FROM IDS.WagonInternalMovement as wim_from --> Дислокация отправка на станцию
	INNER JOIN IDS.WagonInternalRoutes as wir ON wio_from.id_wagon_internal_routes = wir.id --> Текущее внетренее перемещение
	--> Операция отправка на станцию		
	--LEFT JOIN IDS.WagonInternalOperation as wio_from  ON wio_from.[id] = wim_from.[id_wio]
    --LEFT JOIN IDS.WagonInternalMovement as wim_from  ON wio_from.[id] = wim_from.[id_wio]
	INNER JOIN IDS.WagonInternalMovement as wim_from  ON wio_from.[id] = wim_from.[id_wio] -- Включил INNER вместо LEFT чтобы отсеевало записи без wim_from_id (только новые отправки)
	...
	WHERE wio_from.id_operation = @id_operation
	and wio_from.operation_end >= @start and wio_from.operation_end <= @stop

	--and wim_from.[id] is null
   order by wio_from.operation_end desc, wim_from.id_outer_way, wim_from.[num_sostav], wim_from.[position]