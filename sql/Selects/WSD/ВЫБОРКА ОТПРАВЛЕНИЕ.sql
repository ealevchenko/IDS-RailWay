...
SELECT 
	  ...
	  ,wio.[operation_end] as 'Дата окончания Отправления'
	  ,dir_station_amkr.station_abbr_ru as 'Станция отправления'
  ...
  FROM [KRR-PA-CNT-Railway].[IDS].[WagonInternalOperation] as wio
	...
	INNER JOIN IDS.WagonInternalMovement as wim ON wim.[outer_way_start] = wio.operation_end  and  wim.id_wagon_internal_routes = wio.id_wagon_internal_routes
	--> Справочник аренд
  ...
  where wio.[id_operation] in (5) and wio.[operation_start] >= '2025-07-01 00:00:00.000' and wio.[operation_end]<= '2025-10-20 00:00:00.000'
