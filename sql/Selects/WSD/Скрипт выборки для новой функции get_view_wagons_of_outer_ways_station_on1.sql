...
declare @id_station_on int = 11

--select *
--FROM IDS.Directory_OuterWays as outer_ways
--where outer_ways.id_station_on = 11

--SELECT wim1.[num_sostav], 
--count(wim1.[num_sostav]),
--run = (SELECT count(wim.[outer_way_end]) FROM [IDS].[WagonInternalMovement] as wim where wim.[num_sostav]= wim1.[num_sostav] and wim.outer_way_end is not null)
--  FROM [IDS].[WagonInternalMovement] as wim1
--  where wim1.[id_outer_way] in (select id FROM IDS.Directory_OuterWays as outer_ways where outer_ways.id_station_on = 11)
--  group by wim1.[num_sostav]
--  HAVING count(wim1.[num_sostav]) <> (SELECT count(wim.[outer_way_end]) FROM [IDS].[WagonInternalMovement] as wim where wim.[num_sostav]= wim1.[num_sostav] and wim.outer_way_end is not null)

--SELECT count([outer_way_end]) FROM [IDS].[WagonInternalMovement] where [num_sostav]='55-22052023002400' and outer_way_end is not null
...
--  where [id_outer_way] in (select id FROM IDS.Directory_OuterWays as outer_ways where outer_ways.id_station_on = 11)


--SELECT wim1.[num_sostav] FROM [IDS].[WagonInternalMovement] as wim1
--  where wim1.[id_outer_way] in (select id FROM IDS.Directory_OuterWays as outer_ways where outer_ways.id_station_on = 11)
--  group by wim1.[num_sostav]
--  HAVING count(wim1.[num_sostav]) <> (SELECT count(wim.[outer_way_end]) FROM [IDS].[WagonInternalMovement] as wim where wim.[num_sostav]= wim1.[num_sostav] and wim.outer_way_end is not null)
	...
	Left JOIN [IDS].[Directory_Ways] as dir_way_on ON wim_on.[id_way] = dir_way_on.id 
		--> Справочник Станция прибытия реально на путь
  ...
  where wim_from.[num_sostav] in (SELECT wim1.[num_sostav] FROM [IDS].[WagonInternalMovement] as wim1 where wim1.[id_outer_way] in (select id FROM IDS.Directory_OuterWays as outer_ways where outer_ways.id_station_on = @id_station_on) group by wim1.[num_sostav] HAVING count(wim1.[num_sostav]) <> (SELECT count(wim.[outer_way_end]) FROM [IDS].[WagonInternalMovement] as wim where wim.[num_sostav]= wim1.[num_sostav] and wim.outer_way_end is not null))