	...
	declare @stop datetime = Convert(datetime,'2021-09-15 23:59:59',120)
	declare @id_operation int = 5

-- order by [operation_start] desc, [id_outer_way], wim_position
	select * from [IDS].[get_view_wagons_of_operation_send_arrival](@start,@stop) order by 1
	select
	...
	[IDS].[Directory_WagonOperations] as dir_wag_oper_from ON wio_from.id_operation = dir_wag_oper_from.id Left JOIN			-- Справочни Операций над вагонами
	[IDS].[Directory_ConditionArrival] as dir_cond_from ON wio_from.[id_condition] = dir_cond_from.id Left JOIN		-- Справочни Состояние 
    [IDS].[Directory_WagonLoadingStatus] as dir_wls ON wio_from.[id_loading_status] = dir_wls.id Left JOIN			-- Справочни Статус погрузки
	IDS.WagonInternalRoutes as wir ON wio_from.id_wagon_internal_routes = wir.id Left JOIN							-- Справочни Операций над вагонами
	...
	[IDS].[Directory_Station] as dir_station_from ON wim_from.[id_station] = dir_station_from.id Left JOIN			-- Справочни Станция отправки
	[IDS].[Directory_Ways] as dir_way_from ON wim_from.[id_way] = dir_way_from.id Left JOIN							-- Справочни Путь отправки
	...
	--> Прибытие на станцию
	IDS.WagonInternalMovement as wim_on ON wim_from.id = wim_on.parent_id  Left JOIN								-- WIM - прибытия
	[IDS].[Directory_Ways] as dir_way_on ON wim_on.[id_way] = dir_way_on.id Left JOIN							-- Справочни Путь прибытия
	...
	[IDS].[Directory_WagonOperations] as dir_wag_oper_on ON wio_on.id_operation = dir_wag_oper_on.id Left JOIN	-- Справочни Операций над вагонами по прибытию
	[IDS].[Directory_ConditionArrival] as dir_cond_on ON wio_on.[id_condition] = dir_cond_on.id Left JOIN		-- Справочни Состояние 
	--> Прибытие на АМКР
	IDS.ArrivalCars as arr_car ON wir.id_arrival_car = arr_car.id Left JOIN -- Прибытие
	--> Документы по прибытию на АМКР
	...
	IDS.Directory_ConditionArrival as arr_dir_cond ON arr_doc_vag.id_condition =  arr_dir_cond.id Left JOIN		--> Техническое сотояние по прибытию
	IDS.Directory_Cargo as arr_dir_cargo ON arr_doc_vag.id_cargo =  arr_dir_cargo.id Left JOIN				--> Груз по прибытию
	...
	WHERE wio.id_operation = @id_operation and wio.[operation_end] >= @start and wio.[operation_end] <= @stop 