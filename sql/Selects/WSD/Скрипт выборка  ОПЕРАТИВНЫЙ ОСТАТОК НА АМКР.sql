/****** Script for SelectTopNRows command from SSMS  ******/
	...
	select count(id) FROM  [IDS].[WagonInternalRoutes] where [close] is null
	...
	FROM [KRR-PA-CNT-Railway].[IDS].[WagonInternalRoutes] as wir
		...
		-->
		Left JOIN IDS.WagonInternalMovement as wim ON wim.id = (select top(1) [id] FROM [IDS].[WagonInternalMovement] WHERE [id_wagon_internal_routes] = wir.id)
	...
	where wir.[close] is null and  not arr_doc_uz.[klient] = 1 and (not dir_owg.[group] in ('amkr_vz') OR dir_owg.[group] is null)