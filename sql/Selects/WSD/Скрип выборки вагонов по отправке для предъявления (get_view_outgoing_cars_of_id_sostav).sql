...
--select * from [IDS].[get_view_outgoing_cars_of_id_sostav](@id_sostav) order by outgoing_car_position
	...
	FROM [IDS].[OutgoingSostav] as out_sost
		...
		Left JOIN IDS.WagonInternalMovement as wim_wf_pre ON wim_wf_pre.id = (SELECT top(1) [id] FROM [IDS].[WagonInternalMovement] where [id_wagon_internal_routes] = wir.id and [id_filing] is not null order by [id] desc)
		--> Предыдущая подача
