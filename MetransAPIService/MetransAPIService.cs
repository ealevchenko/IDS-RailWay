...
namespace MetransAPIService
{
    ...
    public partial class MetransAPIService : ServiceBase
    {
        ...
        private void InitializeService()
        {
            try
            {
                ...
                this.interval_api_transfer_arrival = int.Parse(ConfigurationManager.AppSettings["IntervalTransferArrival"].ToString());
                ...
                this.active_api_transfer_arrival = bool.Parse(ConfigurationManager.AppSettings["ActiveTransferArrival"].ToString());
                ...
                timer_services_arrival.Interval = this.interval_api_transfer_arrival * 1000;
        ...
        protected override void OnStart(string[] args)
        {
            ...
            //message.WarningLog(servece_name, eventID);message.EventLog(EventStatus.Ok, servece_name, eventID);
            message = String.Format("Интервал выполнения сервиса {0}-{1} сек.", this.thread_api_arrival, this.interval_api_transfer_arrival);
            message.WarningLog(servece_name, eventID);message.EventLog(EventStatus.Ok, servece_name, eventID);
        ...
        protected void Start_APITransferArrival(bool active)
        {
            if (active)
            {
                mtt.Start_APITransferArrival();
            }
        }

        protected void Start_APITransferArrival()
        {
            bool active = bool.Parse(ConfigurationManager.AppSettings["ActiveTransferArrival"].ToString());
            Start_APITransferArrival(active);
        }

        protected void RunTimer_APITransferArrival()
        {
            Start_APITransferArrival();
        ...
        private void OnTimerServices_TransferArrival(object sender, System.Timers.ElapsedEventArgs args)
        {
            ...
            try
            {
                bool active = bool.Parse(ConfigurationManager.AppSettings["ActiveTransferArrival"].ToString());
                Start_APITransferArrival(active);
                if (active != this.active_api_transfer_arrival)
                {
                    this.active_api_transfer_arrival = active;
                    string mes_service_start = String.Format("Сервис : {0}, выполнение потока {1} - {2}", this.servece_name, this.thread_api_arrival, active ? "возабновленно" : "остановленно");
                    mes_service_start.EventLog(EventStatus.Ok, servece_name, eventID);
                }
            }
            catch (Exception e)
